...
function index() {
  fetch(backEndURL)
    .then(r => r.json())
    .then(json => getApi(json.apiKey));
}

function getApi(apiKey) {
  return fetch(`${apiURL}/?key=${apiKey}&q=${question}&${target}`)
    .then(r => r.json())
    .then(r => storeWordsinArr(r))
...
function createScores(lives, user_id) {
  ...
  return fetch(scoreURL, postConfig)
    .then(r => r.json())
    .then(json => console.log(json));
}
...
function assignKoreanWords() {
  const q1 = {
    word: koreanWords[0],
    enword: enWords[0],
    emoji: koreanEmojis[0],
    audio: koreanAudioList[0],
    answer: "oenjjog",
    wrongAnswers: ["oenjjog", "oleunjjog", "jjog", "owenjjog"]
  };
  const q2 = {
    word: koreanWords[1],
    enword: enWords[1],
    emoji: koreanEmojis[1],
    audio: koreanAudioList[1],
    answer: "uisa",
    wrongAnswers: ["elsa", "usa", "uisa", "uija"]
  };
  const q3 = {
    word: koreanWords[2],
    enword: enWords[2],
    emoji: koreanEmojis[2],
    audio: koreanAudioList[2],
    answer: "maegju",
    wrongAnswers: ["soju", "maggeolli", "maegjong", "maegju"]
  };
  const q4 = {
    word: koreanWords[3],
    enword: enWords[3],
    emoji: koreanEmojis[3],
    audio: koreanAudioList[3],
    answer: "hangug",
    wrongAnswers: ["hanpan", "hanjan", "hangug", "hangoop"]
  };
  const q5 = {
    word: koreanWords[4],
    enword: enWords[4],
    emoji: koreanEmojis[4],
    audio: koreanAudioList[4],
    answer: "ssal",
    wrongAnswers: ["seoul", "ssal", "ssul", "ssalsa"]
  };
...
function renderImageForQuestion(langObj) {
  ...
  <div id="en-word" style="font-size:40px;color:#4A9CD5;">${
    langObj.enword
  }</div>`;
  ...
  const answerRadioHTML = `<div class="inputGroup">
      <input type="radio" id="answer1" data-action="answer" class="radio" name="male" value="${
        langObj.wrongAnswers[0]
      }">
      <label for="radio1">${langObj.wrongAnswers[0]}</label>
    </div>
    <div class="inputGroup">
      <input type="radio" id="answer2" data-action="answer" class="radio" name="male" value="${
        langObj.wrongAnswers[1]
      }">
      <label for="radio2">${langObj.wrongAnswers[1]}</label>
    </div>
    <div class="inputGroup">
      <input type="radio" id="answer3" data-action="answer" class="radio" name="male" value="${
        langObj.wrongAnswers[2]
      }">
      <label for="radio3">${langObj.wrongAnswers[2]}</label>
    </div>
    <div class="inputGroup">
      <input type="radio" id="answer4" data-action="answer" class="radio" name="male" value="${
        langObj.wrongAnswers[3]
      }">
...
function endGame() {
  ...
  inputOptions.forEach(node =>
    node.firstElementChild.setAttribute("data-action", "")
  );
}
